...
create table stg_team_xl (team_name, training_time, training_day, training_venue);
insert into stg_team_xl (team_name, training_time, training_day, training_venue) 
  select "Team Name","Training Time","Training Day","Training Venue" from raw_ap_team;
...
insert into stg_mgr_wg (name,fan,qual_coach,first_aid_exp,sg_exp, dbs_exp, team_name, youth_qual_ok, adult_qual_ok)
  select replace("Name","'",''), replace("FAN", "'",''),
...
x.team_name as team, x.role as role, 
g.morc as gs_morc, 
...
from
stg_mgr_xl x full join stg_mgr_dbs d on x.fan = d.fan full join stg_mgr_wg w on w.fan = x.fan full join stg_mgr_gs g on (lower(g.name) = lower(x.name) and g.dob = x.dob);